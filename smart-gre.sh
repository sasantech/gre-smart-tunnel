...
if [[ $1 == "internal-update-haproxy" ]]; then
    ...
    {
        echo "# --- LIGHTNET START ---"
        # اضافه کردن تنظیمات کاهش لاگ در سطح پروکسی
        if [ -f "$PORT_LIST" ]; then
            while read -r line; do
                proto=$(echo "$line" | cut -d: -f1) ; lport=$(echo "$line" | cut -d: -f2) ; rport=$(echo "$line" | cut -d: -f3)
                # اضافه کردن چک‌اینتروال و بی‌صدا کردن لاگ‌های کنسولی
                echo -e "listen forward_${proto}_${lport}\n  bind *:${lport}\n  mode ${proto}\n  server s1 ${target_ip}:${rport} check inter 5000 rise 2 fall 3\n  no log"
            done < "$PORT_LIST"
        fi
        echo "# --- LIGHTNET END ---"
    } > /tmp/lightnet_block
...
# --- UI Interface ---
# غیرفعال کردن نمایش پیام‌های سیستم روی ترمینال جاری
mesg n 2>/dev/null
